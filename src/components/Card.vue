<template>
	<router-link :to="`/${$i18n.locale}/${this.type}/${URL}`" id="card"
		...
		<div class="flex flex-col w-full bg-myBlue-200">
			<div class="bg-myBlue-900 flex flex-col w-full justify-between p-1 text-white">
				<h1 class="text-base md:text-2xl w-full text-center"> {{ title }} </h1>
				
				<h2 class="text-xs md:text-lg text-center italic text-gray-500 font-light">
					{{ year }} - {{ subtitle }} &nbsp;
					<i v-for="(tech, index) in techs" :key="index" :class=tech>&nbsp;</i>
				</h2>
			</div>

			<p class="text-sm md:text-lg p-3 bg-myBlue-200 text-myBlue-900 min-w-full"> {{ content }} </p>
