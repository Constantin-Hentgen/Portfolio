  ...
  <div class="main">
		<main class="w-full flex-col xl:w-1/2 m-auto mt-16 md:mt-14 space-y-8">
			<a id="github" :href="project.github" target="_blank"  class="flex justify-center space-x-2 cursor-pointer p-2 mt-24">
				<h1 class="text-2xl md:text-5xl grid place-items-center text-myBlue-900 font-bold"> {{ $t(`commitment.translatable.title`) }} </h1>
			</a>
...
<script>
	...
	export default {
		...
		methods: {
			fetchContent() {
				this.article = marked(atob(this.$t(`commitment.extendedContent`)))
			}
		}
	}
</script>
